...
int main() {
    ...
    for (int i = 0; i < t_horizon; i++) {
        ...
        if (i < time.size()) {
            dmp.incrementalLearn(phi(i), pos_traj.row(i), vel_traj.row(i), acc_traj.row(i));
        }
        dmp.step();
    }

    std::ofstream file(dmp::test::results_directory + "/3dof_dmp_incremental.csv");
    ...
    gp << "set title '3D Cartesian DMP - Incremental Learning'\n";
